...
- Uses `campsitejs` as the package name (published to npm)
...
1. Set up the monorepo/package structure:
   ...
   - packages/campsitejs/ (the CLI that runs on `npm create`)
   - packages/basecampjs/ (the actual build engine, reusable)

2. Implement the `campsitejs` CLI:
...
4. Build the core engine (basecampjs):
...
Start by generating the full file structure for the monorepo, then provide the code for `packages/campsitejs/index.js` and the template files it will copy.

Make it clean, well-documented, and easy to extend. Use modern ESM syntax and TypeScript if possible (but keep it optional).

- Monorepo layout: core engine in [packages/basecampjs](packages/basecampjs), scaffolder in [packages/campsitejs](packages/campsitejs), example consumer site in [campsite-site](campsite-site).
...
- Core CLI commands (global or via npx): `campsite dev|build|serve` as implemented in [packages/basecampjs/index.js](packages/basecampjs/index.js).
- Build pipeline: loads `campsite.config.js` merging defaults; wipes `outDir`, copies `public/`, walks `src/pages` (skips dotfiles), and processes each file via `renderPage()` in [packages/basecampjs/index.js](packages/basecampjs/index.js).
...
- Template context: `pageContext()` exposes `{ site: { name, config }, page: {...frontmatter, content, source }, frontmatter, content }` for templates (see [packages/basecampjs/index.js](packages/basecampjs/index.js)).
- Server: simple HTTP server with fallback to `index.html`, MIME map for common static assets, default port 4173 (see `serve()` in [packages/basecampjs/index.js](packages/basecampjs/index.js)).
- Dev watch loop: `chokidar` watches `src/` and `public/`; rebuilds are serialized with a pending flag to avoid overlap (see `dev()` in [packages/basecampjs/index.js](packages/basecampjs/index.js)).
...
- Scaffolder (`npm create campsitejs@latest` or `npx campsitejs@latest`): prompts for project name, Markdown support, template engines, package manager, and install flag; protects non-empty targets with a confirm prompt; renames `_gitignore` to `.gitignore`; copies template, swaps markdown vs. nunjucks starter page, prunes Vue/Alpine samples, writes config and dependencies accordingly (see [packages/campsitejs/index.js](packages/campsitejs/index.js)).
- Scaffolder templates: base config in [packages/campsitejs/template/campsite.config.js](packages/campsitejs/template/campsite.config.js), layout + styles in [packages/campsitejs/template/src/layouts/base.njk](packages/campsitejs/template/src/layouts/base.njk) and [packages/campsitejs/template/public/style.css](packages/campsitejs/template/public/style.css), optional Vue/Alpine samples in [packages/campsitejs/template/src/components](packages/campsitejs/template/src/components), and an NJK variant homepage in [packages/campsitejs/template/variants/index.njk](packages/campsitejs/template/variants/index.njk).
- Dependency injection: when scaffolding locally, `basecampjs` is linked via a file: path to the sibling package; otherwise falls back to a semver range.
- Hidden files: the walker skips entries starting with `.`; place generated artifacts elsewhere if they must ship.
- Known gaps: tests and linting scripts are stubs; `renderPage()` currently calls `dirname()` without importing it, so expect a runtime error until `dirname` is added to the path imports in [packages/basecampjs/index.js](packages/basecampjs/index.js).
